...
int main(int argc, char **argv) {
  ...
  exec_sql(db, "CREATE INDEX messages_acquire_lock_test ON messages "
               "(acquire_lock(stream_name))");
  exec_sql(db, "PRAGMA trusted_schema = OFF");
  exec_sql(db, "CREATE VIEW acquire_lock_from_view AS SELECT acquire_lock('account-1')");
  assert_scalar_eq(db, "1", "SELECT COUNT(*) FROM acquire_lock_from_view");
  exec_sql(db,
           "CREATE VIEW stream_version_from_view AS SELECT stream_version('account-1')");
  assert_error(db, "SELECT * FROM stream_version_from_view");
